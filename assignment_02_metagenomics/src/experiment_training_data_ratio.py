...
def main():
    ...
    if len(sys.argv) != 5:
        print("Usage: python3 experiment_training_data_ratio.py training_data.tsv testing_data.tsv ground_truth.tsv output_dir")
        sys.exit(1)
    ...
    for ratio in training_data_ratios:
        print(f"Running experiment for training_data_ratio={ratio}%")
        output_file = os.path.join(output_dir, f"output_ds{ratio}.tsv")

        # Run classifier with specified training data ratio
        classifier = Classifier(training_file, testing_file, output_file)
        classifier.training_data_ratio = ratio
        classifier.run()
        ...
        print(f"AUC-ROC for training_data_ratio={ratio}%: {auc_roc_scores}")
...
def plot_auc_results(auc_results, output_dir):
    """
    Plot AUC-ROC results for different training data ratios.
    """
    ...
    plt.title("AUC-ROC vs. Training Data Ratio")
    plt.xlabel("Training Data Ratio (%)")
    plt.ylabel("Average AUC-ROC")
    plt.grid()
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "auc_roc_vs_training_data_ratio.png"))
    print(f"Plot saved to {os.path.join(output_dir, 'auc_roc_vs_training_data_ratio.png')}")
