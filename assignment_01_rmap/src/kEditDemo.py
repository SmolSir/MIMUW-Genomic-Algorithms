...
def test(rows, cols, edist):
    print(f"test for {rows} rows, {cols} cols, {edist} edist")
    ...
    for diag in range(2, rows + cols + 1):
        ...
        row_end = max(diag - cols - 1, 0 if diag <= cols - rows + edist + 1 else (diag - cols + rows - edist) // 2)
